...
void CPropertiesWnd::OnPaint()
{
	...
	CBrush brush(RGB(0x7f, 0x7f, 0x7f));
	CRect rectCombo;
	m_wndObjectCombo.GetWindowRect(rectCombo);
	ScreenToClient(rectCombo);
	dc.FillRect(rectCombo, &brush);

	CRect rectTB;
	m_wndToolBar.GetWindowRect(rectTB);
	ScreenToClient(rectTB);
	dc.FillRect(rectTB, &brush);
	
	CRect rectPL;
	m_wndPropList.GetWindowRect(rectPL);
	ScreenToClient(rectPL);
	dc.FillRect(rectPL, &brush);	
}
