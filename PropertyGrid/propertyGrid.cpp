...
#define _tmemmove (LPTSTR)memmove
...
#define AllocatedString_Replace(lpszTarget, lpszReplace) \
    (free((lpszTarget)), (lpszTarget) = Nestring(lpszReplace))
...
LPTSTR NestringArray(LPTSTR szzStr)
{
...
LPLISTBOXITEM NewItem(LPTSTR szCatalog, LPTSTR szPropName, LPTSTR szCurValue, LPTSTR szMisc, LPTSTR szPropDesc, INT iItemType)
{
    ...
    lpItem->lpszCatalog = Nestring(szCatalog);
    lpItem->lpszPropName = Nestring(szPropName);
    lpItem->lpszCurValue = Nestring(szCurValue);

    if (PIT_COMBO == iItemType || PIT_EDITCOMBO == iItemType)
        lpItem->lpszMisc = NestringArray(szMisc);
    else
        lpItem->lpszMisc = Nestring(szMisc);

    lpItem->lpszPropDesc = Nestring(szPropDesc);
...
static bool ToolTip_EnumTools(HWND hWnd, WORD nIndex, LPTOOLINFO toolInfo)
{
	return ::SendMessage(hWnd, TTM_ENUMTOOLS, nIndex, (LPARAM)toolInfo);
}
static bool ToolTip_UpdateTipText(HWND hWnd, LPTOOLINFO toolInfo)
{
	return ::SendMessage(hWnd, TTM_UPDATETIPTEXT, 0, (LPARAM)toolInfo);
}
static bool ToolTip_AddTool(HWND hWnd, LPTOOLINFO toolInfo)
{
	return ::SendMessage(hWnd, TTM_ADDTOOL, 0, (LPARAM)toolInfo);
}
static bool ToolTip_NewToolRect(HWND hWnd, LPTOOLINFO toolInfo)
{
	return ::SendMessage(hWnd, TTM_NEWTOOLRECT, 0, (LPARAM)toolInfo);
}
...
LPITEMIDLIST ConvertPathToLpItemIdList(LPTSTR pszPath)
{
    ...
    {
        int len;
        LPWSTR pszWPath = (LPWSTR)_malloca(sizeof(WCHAR) * ((len = strlen(pszPath)) + 1));
        ::MultiByteToWideChar(  CP_ACP, NULL,pszPath, -1, pszWPath,len );
        pDesktopFolder->ParseDisplayName(
         NULL, NULL, pszWPath, &chEaten, &pidl, &dwAttributes);
        pDesktopFolder->Release(/*pDesktopFolder*/);
        _freea( pszWPath );
...
///
/// @returns VOID.
VOID ListBox_OnLButtonUp(HWND hwnd, INT x, INT y, UINT keyFlags)
{
...
///
/// @returns VOID.
VOID ListBox_OnCommand(HWND hwnd, INT id, HWND hwndCtl, UINT codeNotify)
{
    if (ID_BUTTON == id)
    {
        if (NULL == g_lpInst->lpCurrent)
        ...
        switch (g_lpInst->lpCurrent->iItemType)
        {
            ...
            case PIT_FILE:
            {
                ...
                _stscanf(g_lpInst->lpCurrent->lpszMisc,
                    ...
                    _T(     "Path: %256l[^\r\n] ") \
                    _T(     "Filter: %256l[^\r\n] ") \
                    _T(     "Default Extension: %3l[^\r\n] "),
#else
                    _T( "Title: %32[^\r\n] ") \
                    _T(     "Path: %256[^\r\n] ") \
                    _T(     "Filter: %256[^\r\n] ") \
                    _T(     "Default Extension: %3[^\r\n] "),
#endif
...
static VOID Grid_OnSize(HWND hwnd, UINT state, INT cx, INT cy)
{
    ...
    if (NULL != g_lpInst->hwndPropDesc)
    {
		...
		RECT rect = {0, cy - g_lpInst->iDescHeight - 2,
			cx, cy - g_lpInst->iDescHeight};
        RedrawWindow(g_lpInst->hwndParent, &rect, NULL, RDW_ERASE | RDW_INVALIDATE);
    }
    else
    {
...
BOOL Grid_OnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct)
{
    ...
    CHECKED = SELECT = Nestring(_T("T")); //PIT_CHECK select
    UNCHECKED = UNSELECT = Nestring(_T("")); //PIT_CHECK unselect
        ...
        WPARAM wParam, LPARAM lParam)
{
    ...
    return CallWindowProc((WNDPROC)GetProp(hList, TEXT("Wprc")),
